...
static char	*ft_strjoin_m(char *s1, char *s2)
{
	int				i;
	unsigned int	s1_len;
	char			*res;
...
static int	init_l(t_fdf_panel *l, t_fdf_info *p)
{
	...
	l->h_rot_deg = ft_strjoin_m("rotated deg(horizental): ",
			ft_itoa(p->h_angle));
...
void	put_panel(t_fdf_info *p)
{
	...
	mlx_string_put(p->mp, p->wp, 300, 25, 10040319,
		"move map: keyboard cursors");
