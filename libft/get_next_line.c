...
static char	*get_ret_nl(t_gnl **targ_adr, char *buf, t_gnl **head_adr)
{
	char	*ret;
	t_gnl	*target;
	int		nl_idx;
...
static char	*get_ret_nonl(t_gnl **targ_adr, char *buf, t_gnl **head_adr)
{
	char	*ret;
	t_gnl	*target;
...
static char	*get_ret(int r_flag, t_gnl **targ_adr,
		char *buf, t_gnl **head_adr)
{
	t_gnl	*target;
...
char	*get_next_line(int fd)
{
	...
	buf[r_flag] = '\0';
	ret = get_ret(r_flag, &target, buf, &head);
	return (ret);
}
